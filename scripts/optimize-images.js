#!/usr/bin/env node
/*
 * Re-encodes the project images referenced from constants/projects.js as
 ...
 * replaces the source file; PNG sources are removed and constants/projects.js
...
const PROJECTS_FILE = path.join(ROOT, 'constants', 'projects.js');
...
async function optimize(file, kind) {
  ...
  if (!needsResize && before <= MIN_BYTES) {
    console.log(
      `keep   ${file} (${meta.width}x${meta.height}, ${kb(before)})`,
    );
    return { outFile: file, saved: 0 };
  }
  ...
  console.log(
    `write  ${file} -> ${outFile}: ${meta.width}x${meta.height} ${kb(before)}` +
...
(async () => {
  ...
  ].map((m) => ({ kind: m[1] ? 'skinny' : 'full', file: m[2] }));
