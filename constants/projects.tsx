...
const approximateUserCount = (users: number): string | null => {
  if (users >= 1000000) {
...
): Pick<Project, 'usersFormatted' | 'usersApproximate'> => {
  if (!users) return {};
  return {
...
export const projectsList: Project[] = _projects.map((project, i) => {
  const prevId =
    i === 0
...
const projectsById: Record<string, Project> = projectsList.reduce(
